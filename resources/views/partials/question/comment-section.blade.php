<?php $comment_limit = 3; ?>
<article class="row row-cols-2 mb-1 my-5">
  <div class="col-1 flex-wrap me-0 pe-0"> </div>
  <div class="col-11 col-sm-11 col-md-11 col-lg-11 flex-wrap pe-4 ps-0">
    <?php
    $comment_count = 0;
    foreach ($comments as $comment) {
    $comment_count++; ?>
    <div
      class="<?php if ($comment_count > $comment_limit) { ?> collapse hidden{{ $id }} <?php } ?>">
      <div class="comment d-flex justify-content-between shadow-sm border border-2 mb-1 px-2 bg-light rounded">
        ...
        <blockquote class="blockquote mb-0">
          <p class="card-text mb-0"><small class="text-muted">{{ $comment->content->creation_date }}&nbsp;<a
                class="signature" href="#">{{ $comment->content->author_id }}</a></small></p>
        </blockquote>
      </div>
    </div>
    <?php
    ...
    <a class="show-more text-sky me-2 mt-2" data-bs-toggle="collapse" href=".hidden{{ $id }}"
      aria-expanded="false">Show {{ $comment_count - $comment_limit }} more comments</a>
    <a class="show-less text-sky me-2 mt-2" data-bs-toggle="collapse" href=".hidden{{ $id }}"
      aria-expanded="false">Hide {{ $comment_count - $comment_limit }} last comments</a>
    <?php }
    ?>
    ...
    <a class="float-end btn blue-alt extra text-white add-comment mt-3" data-bs-toggle="collapse"
